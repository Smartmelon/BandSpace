...
### 时间偏移量
偏移值即为音频文件开头至第一个拍子的空白时间，音频将从偏移值位置开始播放。
该值仅影响编辑谱面时的音频起点位置，实际游戏内仍从音频原起始点播放。
...
### 视频
当场景选择**舞台**时，还可以上传**.mp4**格式的视频文件（上限200M），该视频将在舞台场景的LED屏幕中播放。如果您没有视频文件也不用担心，留空即可，我们会自动播放提前为您准备好的舞台视频。

### 乐器与难度
超弦乐队支持四种乐器玩法：鼓、键盘、贝斯、吉他。您可以为歌曲制作所有乐器共计4个谱面，也可以只设计其中的几种乐器谱面。因为每个乐器玩法不同，所以它们的谱面编辑界面和规则各有一些不同。  
您可以在下文**谱面编辑>乐器区别**中找到您需要的乐器进行了解。  
...
**移动音符**：使用鼠标覆盖已有的单个音符，使用**Alt+鼠标左键**后可抓住并移动该音符，松开**鼠标左键**以放置到新位置。  
**选择音符**：使用**Shift+鼠标左键**点击音符，可以选中该音符；在有音符选中时，**Ctrl+A** 取消全部音符的选中状态。  
**链接长音符** ：在选中多个音符时，可使用快捷键**L**以这些选中的音符作为节点生成长音符。请注意不同乐器的长音符规则，不符合规则时会生成失败。    
**删除**：  
方法一. 使用左侧工具栏中的删除工具以进入删除模式，在该模式下使用**鼠标左键**点击音符以删除该音符；  
方法二. 在选中音符时，可使用**Del**键快速删除选中的所有音符。  
...
**普通音符**：显示为黄色，玩家使用任意力度敲击即可得分。  
**重击音符**：显示为紫色，玩家需要大力敲击来得到满分，但是只要击中了就会根据敲击的力度得分。  
...
当选择双指音符时，可在1&2、3&4、5&6轨放置需同时按压trigger和grip键双指音符。  
